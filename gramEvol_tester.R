...
# equivalent using string expr instead of R expr
ruleDef.alt <- list(expr = gsrule("<expr><op><expr>"
                              , "<func>(<expr>)"
                              , "<var>")
                , func = gsrule("sin", "cos"
                                , "log", "sqrt")
                ...
                , var = grule(distance, distance^n, n)
                , n = grule(1, 2, 3, 4))
CreateGrammar(ruleDef.alt)

planets <- c("Venus", "Earth"
             , "Mars", "Jupiter"
             , "Saturn", "Uranus")
distance <- c(0.72, 1.00, 1.52
              , 5.20, 9.53, 19.10)
period <- c(0.61, 1.00, 1.84, 11.90
            , 29.40, 83.50)
